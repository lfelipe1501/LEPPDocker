...
COPY ./config/nginx/nginx.zip /etc/nginx.zip
COPY ./app/default /var/www/html/default

# Copy EntryPoint Configuration
COPY ./config/nginx/start.sh /start.sh
COPY ./config/nginx/app.ini /etc/nginx-ui/app.ini
